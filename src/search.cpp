...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[~us][(ss->ply - 1) / 8][((ss - 1)->currentMove).from_to()]
          << bonus * 1151 / 1024;
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else
            {
                ...
                history += 2 * thisThread->mainHistory[us][ss->ply / 8][move.from_to()];
        ...
        if (capture)
            ...
            ss->statScore = 2 * thisThread->mainHistory[us][ss->ply / 8][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    Color us = pos.side_to_move();
    workerThread.mainHistory[us][ss->ply / 8][move.from_to()]
